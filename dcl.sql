...
--사용자 생성, TABLESPACE 지정 및 권한부여를 일괄적으로 수행
CREATE USER C##SEMI2
IDENTIFIED BY SEMI2
DEFAULT TABLESPACE USERS
TEMPORARY TABLESPACE TEMP PROFILE DEFAULT;

GRANT CONNECT RESOURCE TO C##SEMI2;
GRANT CREATE VIEW, CREATE SYNONYM TO C##SEMI2;
GRANT UNLIMITED TABLESPACE TO C##SEMI2;
ALTER USER C##SEMI2 ACCOUNT UNLOCK;
...
DROP USER C##SEMI2 CASCADE


--트랜잭션
--SAVEPOINT 이름 : 현재까지의 트랜잭션을 특정 이름으로 지정하는 명령(여러군데로 지정)
--ROLLBACK TO 이름 :  저장되지 않은 데이터를 모두 취소하고 트랜잭션을 종료(그 지점부터 다시)
--COMMIT : 저장되지 않은 모든 데이터베이스를 저장하고 핸져의 트랜잭션을 종료
